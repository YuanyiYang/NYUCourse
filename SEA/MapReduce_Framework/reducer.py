...
class Reduce(web.RequestHandler):
    ...
    def get(self):
        ...
        for i in range(numMappers):
            ...
            params = urllib.urlencode({'reducerIx':reducerIx,'mapTaskIDs':mapTaskIDs[i]})
            url = "http://%s/retrueveMapOutput?%s" % (server, params)
...
def reducer(kvPairs, reducerPath, reducerIx, outputFile):
    ...
    p = subprocess.Popen([reducerPath, str(reducerIx)], stdin=subprocess.PIPE, stdout=output)
    for pair in kvPairs:
